...
const entry = await getEntry('en', 'etrading-consulting');
if (!entry) throw new Error('Content not found: en/etrading-consulting');
...
<BaseLayout
  ...
  navKey={data.navKey}
>
  <Hero eyebrow={data.eyebrow} headline={data.headline} subheadline={data.subheadline} />
  ...
  {data.related && data.related.length > 0 && (
    <RelatedServices lang="en" relatedKeys={data.related} />
  )}

  {data.faq && data.faq.length > 0 && (
    <Faq lang="en" items={data.faq} />
  )}
...
<style>
  ...
  .service-content :global(h2:first-child) { margin-top: 0; }
